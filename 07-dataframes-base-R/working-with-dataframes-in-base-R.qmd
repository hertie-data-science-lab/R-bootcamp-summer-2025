...
## Dataframes with Base

While you'll work with dataframes with [`dplyr`](../05-dplyr/working-with-dataframes-with-dplyr.html) throughout Intro to Data Science, it's also possible to work with them using only base R.

## Dataframe Columns as Vectors

Dataframe columns can be accessed as vectors using `[[]]`, `$`, or `[,]` (the leading comma tells R you are selecting columns and not rows--dataframes use `[row,column]` like matrices).
...
Without using `dplyr` (e.g. `summarise()`),
