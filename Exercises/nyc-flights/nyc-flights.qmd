...
```{r}
on_time <- flights %>%
  filter(month == 6 | month == 7 | month == 8 & arr_delay <= 0)
```
/
...
destinations <- flights %>% 
  group_by(origin) %>% 
  summarise(n_origin = n_distinct(dest))
```
...
```{r}
###### à faire 
n_flights_2013 <- flights %>% 
  filter(year == 2013) %>% 
  group_by(origin, month) %>% #added analysis per month too
  summarise(n_flights = n_distinct(flight)) %>% 
  ungroup()

#it follows the same pattern as number of destinations imo
```
...
```{r}
delay_rate_df <- flights %>% 
  mutate(delayed_departure = dep_delay > 0) %>% 
  summarise(prop_delayed_flights = mean(delayed_departure, na.rm=T))
...
```{r}
prop_delay_origin <- flights %>% 
  group_by(origin) %>% 
  mutate(delayed_departure = dep_delay > 0) %>% 
  summarise(prop_delayed_flights = mean(delayed_departure, na.rm=T)) %>% 
  ungroup()

prop_delay_carrier <- flights %>% 
  group_by(carrier) %>% 
  mutate(delayed_departure = dep_delay > 0) %>% 
  summarise(prop_delayed_flights = mean(delayed_departure, na.rm=T)) %>% 
  ungroup()
...
```{r}
origin_carrier_extended <- origin_carrier %>% 
  group_by(origin) %>% 
  mutate(origin_prop_delayed = sum(n_flights * prop_delayed_flights) / sum(n_flights)) %>% 
  group_by(carrier) %>% 
  mutate(carrier_prop_delayed = sum(n_flights * prop_delayed_flights) / sum(n_flights))
```
...
```{r}
origin_carrier_extended <- origin_carrier_extended |>
  filter(prop_delayed_flights < origin_prop_delayed, prop_delayed_flights < carrier_prop_delayed)
```
